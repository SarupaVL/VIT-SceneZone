...
gemini_api.api_key = os.getenv('GEMINI_API_KEY')

if not gemini_api.api_key:
    raise ValueError("Gemini API key not found. Please set it in the environment variables.")

# Define a function to preprocess the chat messages
def preprocess_messages(chat_string):
    """Preprocesses chat messages by extracting the date, sender, and message text.

    Args:
        chat_string (str): A string containing the chat messages.

    Returns:
        list: A list of preprocessed chat messages in the format:
            [
                {"date": "2024-09-14", "sender": "+91 62320 98300", "message": "Hey everyone, are we still on for the movie night this weekend? I’m thinking of doing a little popcorn bar setup with different toppings."},
                # ...
            ]
    """

    # Regular expression to match chat messages
    message_pattern = r"\[(\d{2}:\d{2} \w{2}, \d{2}/\d{2}/\d{4})] (\+\d{12}): (.+)"

    # Extract chat messages from the string
    matches = re.findall(message_pattern, chat_string)

    # Create a list of preprocessed chat messages
    preprocessed_messages = []
    for match in matches:
        timestamp, sender, message = match
        date = timestamp.split(", ")[1]
        preprocessed_messages.append({"date": date, "sender": sender, "message": message})

    return preprocessed_messages

# Receive the chat messages as a string
chat_string = """[11:39 am, 14/9/2024] +91 62320 98300: Hey everyone, are we still on for the movie night this weekend? I’m thinking of doing a little popcorn bar setup with different toppings.
...
preprocessed_messages = preprocess_messages(chat_string)

# Send the preprocessed chat messages to the Gemini API
# Use the Gemini API to generate a summary
response = gemini_api.generate_content(
    prompt="Summarize the following conversation:",
    text="\n".join([f"{message['date']} {message['sender']}: {message['message']}" for message in preprocessed_messages]),
    max_tokens=4000,
)

# Process the response from the Gemini API
summary = response.text
print("yoo here is the summary")
print(summary)